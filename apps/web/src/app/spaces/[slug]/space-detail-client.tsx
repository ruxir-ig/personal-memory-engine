...
export function SpaceDetailClient({ slug: slugOverride }: { slug?: string } = {}) {
  ...
  return (
    <>
      <PageHeading
        ...
        title={space.title}
        copy={space.description ?? "Everything Quipu filed into this space."}
      />
      {items.length === 0 ? (
        <EmptyState title="Nothing here yet" icon={<Library size={20} />}>
          When a saved note, link, file, or task matches this category, Quipu will place it here automatically.
        </EmptyState>
      ) : (
