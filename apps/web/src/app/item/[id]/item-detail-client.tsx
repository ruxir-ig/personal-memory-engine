...
export function ItemDetailClient() {
  ...
  useEffect(() => {
    ...
    async function loadRaw() {
      if (!record.data?.artifact.originalPath) return;
      const blob = await getVaultBlobByKey(record.data.artifact.originalPath);
      if (!blob) return;
      url = blobUrl(blob.bytes, blob.mimeType);
      setRawUrl(url);
    }
    void loadRaw();
    return () => {
  ...
  const { artifact, space, summary, intents, reminders } = record.data;
  const meta = kindMeta[artifact.kind] ?? kindMeta.note;
  const s = artifact.structured ?? {};
  const url = str(s.url);
  const sourceText = record.data.chunks.map((chunk) => chunk.text).join("\n\n");
      ...
      <PageHeading
        ...
        actions={
          <div className="head-actions">
            ...
            {rawUrl ? (
              <a className="btn secondary sm" href={rawUrl} download={artifact.sourceLabel}>
                <Download size={15} />
                Raw file
              </a>
            ) : null}
        ...
        <div className="stack">
          {artifact.kind === "credential" ? (
            <section className="card pad">
              ...
              <SecretCard label={str(s.secretLabel) ?? artifact.title} masked={str(s.secretMasked) ?? "\u2022\u2022\u2022\u2022\u2022\u2022"} secretVaultId={str(s.secretVaultId)} service={str(s.service)} />
