import { getAgentToolForRun, listAgentTools, setAgentToolEnabled, upsertAgentTool } from "@/client/memory/agent-workspace";
  ...
  try {
    ...
    if (args.action === "run_tool") {
      ...
      return {
        toolId: "toolkit",
        ok: true,
        summary: `Loaded custom tool instructions: ${tool.name}.`,
        data: { customTool: tool },
      };
    }
    ...
    return {
      toolId: "toolkit",
      ok: false,
      summary: "Agent toolkit failed.",
      data: { args },
      error: error instanceof Error ? error.message : "Unknown toolkit error",
    };
  }
}
