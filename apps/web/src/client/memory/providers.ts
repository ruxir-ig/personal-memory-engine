...
import { getEnvProvider, type OpenAICompatibleProvider } from "@/client/ai/provider";
import { decryptIfEncrypted, encryptString, isVaultUnlocked, parseEncrypted, serializeEncrypted } from "@/client/vault/crypto-vault";
import { type MemoryData, type StoredProvider, now, providerForClient, readData, writeData } from "./store";

async function resolveProviderApiKey(storedKey: string): Promise<string | null> {
  if (!parseEncrypted(storedKey)) return storedKey;
  if (!isVaultUnlocked()) return null;
  try {
    return await decryptIfEncrypted(storedKey);
  } catch {
    return null;
  }
}

export async function getDefaultAiProvider(data: MemoryData): Promise<OpenAICompatibleProvider | null> {
  const stored =
  ...
  if (stored) {
    const apiKey = await resolveProviderApiKey(stored.apiKey);
...
export async function upsertProvider(input: ProviderInput): Promise<ModelProviderRecord> {
  if (!isVaultUnlocked()) {
    throw new Error("Unlock your vault in Settings before saving provider API keys.");
  }
  const data = await readData();
  ...
  const encryptedKey = serializeEncrypted(await encryptString(input.apiKey));
