...
function findList(lists: TodoList[], args: { listId?: string; listTitle?: string }) {
  ...
  const wanted = normalizeTitle(args.listTitle || defaultListTitle()).toLowerCase();
...
export async function listTodos(args: { listId?: string; listTitle?: string; status?: TodoStatus | "all"; includeArchivedLists?: boolean } = {}) {
  ...
  return data.todos
    ...
    .sort((a, b) => {
      const statusRank = { open: 0, done: 1, cancelled: 2 } satisfies Record<TodoStatus, number>;
      const priorityRank = { high: 0, normal: 1, low: 2 } satisfies Record<TodoPriority, number>;
      return statusRank[a.status] - statusRank[b.status] || priorityRank[a.priority] - priorityRank[b.priority] || (a.dueAt ?? "9999").localeCompare(b.dueAt ?? "9999") || b.createdAt.localeCompare(a.createdAt);
    })
...
async function ensureTodoList(data: Awaited<ReturnType<typeof readData>>, args: { listId?: string; listTitle?: string }) {
  ...
  const list: TodoList = {
    id: randomUUID(),
    title: normalizeTitle(args.listTitle || defaultListTitle()),
    createdAt: timestamp,
    updatedAt: timestamp,
  };
  data.todoLists.push(list);
...
export async function addTodoItem(args: {
  title: string;
  listId?: string;
  listTitle?: string;
  notes?: string;
...
export async function updateTodoItem(args: {
  itemId: string;
  title?: string;
  notes?: string;
  status?: TodoStatus;
...
export async function upsertAgentTool(args: {
  toolId?: string;
  name: string;
  summary: string;
  whenToUse: string;
  ...
  if (existing) {
    ...
    existing.whenToUse = args.whenToUse.trim();
    existing.instructions = args.instructions.trim();
    existing.inputSchema = args.inputSchema ?? existing.inputSchema;
